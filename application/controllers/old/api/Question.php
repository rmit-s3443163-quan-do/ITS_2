...
class Question extends REST_Controller {
    ...
    public function index_get() {
        ...
        if ($data = $this->question_m->get(['id' => $id])) {
    ...
    public function index_post() {
        if (!$this->post('text')) {
            ...
            $data['type'] = $this->post('question_type')?$this->post('question_type'):0;
            $data['time'] = time();
        }

        if ($id = $this->question_m->add($data))
            $this->response([
                    'id' => $id,
                    'text' => $this->post('text')]
                , 200);
    ...
    public function index_put() {
        ...
        if ($id = $this->question_m->update($data)) {
